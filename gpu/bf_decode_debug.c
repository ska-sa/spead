...
#ifdef DEBUG
 fprintf(stderr,"Tranpose: %i channels, %i rows, %i data_len\n",channels,rows,data_len);
#endif
 blocksize = 8;
  // keep this fixed !!
...
int spead_api_callback(struct spead_api_module_shared *s, struct spead_item_group *ig, void *data)
{
  ...
  char timestampFileName[255];
  char offsetFileName[255];

  snprintf(timestampFileName, 255, "%s.ts",ss->filename);
  snprintf(offsetFileName,255,  "%s.ofs",ss->filename);

   fprintf(stderr, "Opening new file %s with offset base %llu\n",ss->filename, ss->prior_ts);
   fprintf(stderr, "Opening timestamp new file %s with offset base %llu\n",timestampFileName, ss->prior_ts);
   fprintf(stderr, "Opening new offset file %s with offset base %llu\n",offsetFileName, ss->prior_ts);
   if (w_fd2 != NULL) {
    close(w_fd2);
    close(timestampfile);
    close(offsetfile);
   ...
   timestampfile = open(timestampFileName, O_WRONLY | O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO);
   
   offsetfile = open(offsetFileName, O_WRONLY | O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO);
  ...
  // fprintf(stderr, "before write_debug\n");
  if (write_debug_data(ts, (ts - ss->prior_ts) / 4 / (ss->fullband ? 1 : 2), itm->i_data_len) < 0) {
    fprintf(stderr,"%s: failed to write debug data to disk\n",__func__);
  }
