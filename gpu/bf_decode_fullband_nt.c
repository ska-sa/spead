...
void old_transpose_bf_block(void *input,void *output, uint64_t data_len, uint64_t stride, uint64_t bytes_per_sample) {
 uint16_t * in;
 uint16_t * out;
...
/* for (ts=0; ts < rows; ts+=blocksize) {
  for (chan=0; chan < cols; chan+=blocksize) {
   offset_in = ts + (chan*rows);
   offset_out = chan + (ts*cols);
   for (k=0; k < blocksize; k++) {
    //for (j=0; j < blocksize; j++) {
     //out[offset_out + j + (k*cols)] = in[offset_in + k + (j*rows)];
    
    inner_temp = k*rows;
    // single cache line worth of unrolling
    out[offset_out + k] = in[offset_in + 0];
    out[offset_out + k + (1*cols)] = in[offset_in + 1 + inner_temp];
    out[offset_out + k + (2*cols)] = in[offset_in + 2 + inner_temp];
    out[offset_out + k + (3*cols)] = in[offset_in + 3 + inner_temp];
    out[offset_out + k + (4*cols)] = in[offset_in + 4 + inner_temp];
    out[offset_out + k + (5*cols)] = in[offset_in + 5 + inner_temp];
    out[offset_out + k + (6*cols)] = in[offset_in + 6 + inner_temp];
    out[offset_out + k + (7*cols)] = in[offset_in + 7 + inner_temp];
    //}
   }
  }
 }*/
...
int spead_api_callback(struct spead_api_module_shared *s, struct spead_item_group *ig, void *data)
{
  ...
  if (ss->prior_ts == 0) {
   ...
   if (ss->flag[3] == 1) { snprintf(ss->filename,100,"/data2/%llu.dat",ts); ss->flag[3]=1; }
