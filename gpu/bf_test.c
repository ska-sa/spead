...
int spead_api_callback(struct spead_api_module_shared *s, struct spead_item_group *ig, void *data)
{
  ...
  tsp = &ts;
  if (pwrite(w_fd2,tsp,sizeof(unsigned long long), ((ts - ss->prior_ts) / 4) * sizeof(unsigned long long)) < 0) {
   fprintf(stderr,"Failed to write sample counter to disk\n");
  }
